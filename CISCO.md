...
[R2] - 목적,출발지에 따라 전송,반송 해주기 위해 스테틱.
...
PC에서 루프백, 루프백에서 PC로 통신도 하고싶어서 적다보니 좀 길어져서 그런데, 방향을 하나 정하고, 그 방향으로 가는건 전부 디폴트로 처리하고, 나머진 스테틱으로 직접 처리해도 돼?

예를 들자면 1번 라우터는... 어차피 반대 방향엔 PC 밖에 없으니 넘어가고
2번 라우터는 3번 라우터가 있는 방향은 디폴트로 하고, 그 반대인 1번 라우터 쪽은 1번 라우터으 