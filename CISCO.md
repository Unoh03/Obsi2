...
!DHCP 할 때 DNS 서버를 네이버로 할 예정이니 네이버를 경유하여 구글로 갈 수 있게 네이버에 구글도 추가.
...
!이게 제일 헷갈려. 같이 올린 이미지 보고 DNS 보충 설명 할 준비 해줘.
```
## **1. 인터페이스 추가**
```sh
[R1]
conf t
in g0/0
ip ad 192.168.0.254 255.255.255.0
no sh
in g0/1
ip ad 12.12.12.1 255.255.255.252
no sh

[R2]
conf t
in g0/0
ip ad 12.12.12.2 255.255.255.252
no sh
in g0/2
ip ad 23.23.23.1 255.255.255.252
no sh
in g0/1
ip ad 2.2.2.14 255.255.255.240
no sh

[R3]
conf t
in g0/0
ip ad 23.23.23.2 255.255.255.252
no sh
in g0/2
ip ad 34.34.34.1 255.255.255.0
no sh
in g0/1
ip ad 3.3.3.6 255.255.255.248
no sh

[R4]
conf t
in g0/0
ip ad 34.34.34.2 255.255.255.0
no sh
in g0/1
ip ad 172.16.0.254 255.255.255.0
no sh
...
[R1]
!g0/1 포트의 IP 를 외부 ip로 쓰라 함 + PC(인 척하는 서버) 2개 = NAT-PAT 사용. 근데 pool 대신 포트를 넣은.
conf t
